const menu = [
  ...
  const starterItems=document.getElementById('starters');
  const starter=menu.filter(item => item.category==='starters');
  const mainItems=document.getElementById('main');
  const main=menu.filter(item => item.category==='mains');
  const dessertItems=document.getElementById('desserts');
  const dessert=menu.filter(item => item.category==='desserts');
  const beverageItems=document.getElementById('beverages');
  const beverage=menu.filter(item => item.category==='beverages');

  
 
  function showItems(items){
    menuItems.innerHTML = ``;
    console.log("***showItems menu***:",items);
    items.forEach(item =>{
      const div = document.createElement('div');
      
      div.classList.add('menu','col-12', 'col-md-6', 'col-lg-4', 'mb-4', 'd-flex' ,'justify-content-center');
      div.innerHTML +=`<div>
...
allItems.addEventListener('click',() => {
  ...
  starterItems.addEventListener('click',()=>{
    showItems(starter);
  });
  ...
  dessertItems.addEventListener('click',()=>{
    showItems(dessert);
  });
     
  beverageItems.addEventListener('click', ()=>{
    showItems(beverage);
  });
  
  searchBar.addEventListener('keyup', (e)=>{
    const value = e.target.value.toLowerCase();
    const filteredItems = menu.filter(item => item.title.toLowerCase().includes(value));
